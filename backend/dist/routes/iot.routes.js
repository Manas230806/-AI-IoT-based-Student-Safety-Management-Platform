...
router.post('/face-scan', async (req, res) => {
    ...
    if (status === 'SUCCESS') {
        try {
            ...
            if (!validBusId) {
                const firstBus = await prisma_1.prisma.bus.findFirst();
                if (firstBus) {
                    validBusId = firstBus.id;
                }
                else {
                    // Create a demo bus and school if they don't exist
                    const school = await prisma_1.prisma.school.findFirst() || await prisma_1.prisma.school.create({
                        data: { name: 'Demo School', address: 'Demo Address', contactInfo: '123' }
                    });
                    const newBus = await prisma_1.prisma.bus.create({
            ...
            // 2. Save to Database
            if (studentId) {
                const attendanceRecord = await prisma_1.prisma.attendanceRecord.create({
                    data: {
                        studentId,
                        busId: validBusId,
                        type: activityType,
                ...
                index_1.io.emit('attendanceUpdate', {
            ...
            (async () => {
                try {
                    ...
                    if (!recipientEmail && studentId) {
                        const studentWithParent = await prisma_1.prisma.student.findUnique({
                            where: { id: studentId },
                    ...
                    if (!recipientEmail && name) {
                        ...
                        const studentByName = await prisma_1.prisma.student.findFirst({
